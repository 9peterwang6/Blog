...
facility_set *terms;     // shuttle queues at each stop
...
int find_the_crowd(); //find the stop that has the longest queue
double avg_qlen(); //find the current average queue length in every stop 
...
{ 
  ...
  terms = new facility_set("terminal stop", terminals +1);
  create("sim");
...
double avg_qlen()
{
  ...
  for(int i=0; i<=terminals; i++)
  {
   sum += (*buttons)[i].qlength();
  }
  return double(sum)/(terminals);
}
...
int find_the_crowd()
{
  ...
  {
    if((*buttons)[i].qlength() > qlen)
    {
      stop = i;
      qlen = (*buttons)[i].qlength();
    }
  }
  return stop;
}
...
void loop_around_airport(long & seats_used, long route) { // one trip around the airport
  ...
  (*terms)[CARLOT].reserve();
  load_shuttle(CARLOT, seats_used);
  shuttle_occ.note_value(seats_used);
  (*terms)[CARLOT].release();  
